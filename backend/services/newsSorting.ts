...
import { newsAPIResponse, newsData, uniqueSourceIndices } from '../types/types';

// Object mapping a news site url to its name used in News API
export const sourceURLMap : Record<string, string>= {
    'vox.com': 'Vox',
    'cnn.com': 'CNN',
    'msnbc.com': 'MSNBC',
 ...
 */
function _getUniqueSourceIndices(newsData: newsAPIResponse) : [uniqueSourceIndices, Set<string>] {
    const uniqueSourcesSeen : Set<string> = new Set();
    const uniqueSourceIndices : uniqueSourceIndices = {};
 ...
 */
export function maximizeNewsDiversity({ newsData, numStories = 5 } : { newsData: newsAPIResponse, numStories?: number }): [newsData[], Set<string>] {
    const [uniqueSourceIndices, uniqueSourcesSeen] = _getUniqueSourceIndices(newsData);
    const newsSourcesUsed : Set<string> = new Set();
